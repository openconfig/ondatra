...
// NewRSVP returns a new RSVP configuration.
// Tests should not call this directly; call Interface.AddRSVP() instead.
func NewRSVP(pb *opb.RsvpConfig) *RSVP {
	return &RSVP{pb}
}

// RSVP represents an RSVP config on an ATE.
// Must be constructed by calling NewRSVP().
type RSVP struct {
	pb *opb.RsvpConfig
}
...
func (r *RSVP) EndpointPB() *opb.Flow_Endpoint {
	return &opb.Flow_Endpoint{
		InterfaceName: r.pb.GetInterfaceName(),
		Generated:     &opb.Flow_Endpoint_RsvpName{RsvpName: r.pb.GetName()},
	}
}
